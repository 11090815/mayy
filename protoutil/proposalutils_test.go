...
func createCIS() *ppeer.ChaincodeInvocationSpec {
	return &ppeer.ChaincodeInvocationSpec{
		ChaincodeSpec: &ppeer.ChaincodeSpec{
			Type:        ppeer.ChaincodeSpec_GOLANG,
			ChaincodeId: &ppeer.ChaincodeID{Name: "chaincode_name"},
			Input:       &ppeer.ChaincodeInput{Args: [][]byte{[]byte("arg1"), []byte("arg2")}},
		},
	}
}
...
func TestGetChaincodeDeploymentSpec(t *testing.T) {
	...
	cds, _ := proto.Marshal(&ppeer.ChaincodeDeploymentSpec{
		ChaincodeSpec: &ppeer.ChaincodeSpec{
			Type: ppeer.ChaincodeSpec_GOLANG,
		},
	})
...
func TestCDSProposals(t *testing.T) {
	...
	cds := &ppeer.ChaincodeDeploymentSpec{
		ChaincodeSpec: &ppeer.ChaincodeSpec{
			Type: ppeer.ChaincodeSpec_GOLANG,
		},
	}
	...
	// install
	prop, txid, err = protoutil.CreateInstallProposalFromChaincodeDeploymentSpec(cds, creator)
	...
	// deploy
	prop, txid, err = protoutil.CreateDeployProposalFromChaincodeDeploymentSpec(chainID, cds, creator, policy, escc, vscc, nil)
	require.NotNil(t, prop, "Deploy proposal should not be nil")
	require.NoError(t, err, "Unexpected error creating deploy proposal")
	require.NotEqual(t, "", txid, "txid should not be empty")

	// upgrade
	prop, txid, err = protoutil.CreateUpgradeProposalFromChaincodeDeploymentSpec(chainID, cds, creator, policy, escc, vscc, nil)
...
func TestProposal(t *testing.T) {
	...
	if chdr.Type != int32(pcommon.HeaderType_ENDORSER_TRANSACTION) ||
		shdr.Nonce == nil ||
	...
	// sanity check on cis
	if cis.ChaincodeSpec.Type != ppeer.ChaincodeSpec_GOLANG ||
...
func TestProposalResponse(t *testing.T) {
	events := &ppeer.ChaincodeEvent{
		ChaincodeId: "ccid",
		EventName:   "EventName",
		Payload:     []byte("EventPayload"),
		TxId:        "TxID",
	}
	ccid := &ppeer.ChaincodeID{
		Name:    "ccid",
		Version: "v1",
	}

	pHashBytes := []byte("proposal_hash")
	pResponse := &ppeer.Response{Status: 200}
	...
	prpBytes, err := protoutil.GetBytesProposalResponsePayload(pHashBytes, results, eventBytes, pResponse, ccid)
	if err != nil {
		t.Fatalf("Failure while marshalling the ProposalResponsePayload")
		return
	}
	...
	event, err := protoutil.UnmarshalChaincodeEvent(act.Events)
	if err != nil {
	...
	pr := &ppeer.ProposalResponse{
		Payload:     prpBytes,
		Endorsement: &ppeer.Endorsement{Endorser: []byte("endorser"), Signature: []byte("signature")},
		Version:     1, // TODO: pick right version number
		Response:    &ppeer.Response{Status: 200, Message: "OK"},
	}
...
func TestEnvelope(t *testing.T) {
	var signer = &mocks.Signer{}
	signer.SetSerializeReturns([]byte("creator"), nil)
	signer.SetSignReturns([]byte("signature"), nil)
	creator, err := signer.Serialize()
	require.NoError(t, err)

	prop, _, err := protoutil.CreateChaincodeProposal(pcommon.HeaderType_ENDORSER_TRANSACTION, testChannelID, createCIS(), creator)
	if err != nil {
		t.Fatalf("Could not create chaincode proposal, err %s\n", err)
		return
	}

	response := &ppeer.Response{Status: 200, Payload: []byte("payload")}
	result := []byte("res")
	ccid := &ppeer.ChaincodeID{Name: "foo", Version: "v1"}

	presp, err := protoutil.CreateProposalResponse(prop.Header, prop.Payload, result, nil, response, ccid, signer)
	if err != nil {
	...
	tx, err = protoutil.UnmarshalEnvelope(envBytes)
	if err != nil {
	...
	if !bytes.Equal(sh.Creator, creator) {
...
func TestInvokedChaincodeName(t *testing.T) {
	t.Run("Success", func(t *testing.T) {
		name, err := protoutil.InvokedChaincodeName(protoutil.MarshalOrPanic(&ppeer.Proposal{
			Payload: protoutil.MarshalOrPanic(&ppeer.ChaincodeProposalPayload{
				Input: protoutil.MarshalOrPanic(&ppeer.ChaincodeInvocationSpec{
					ChaincodeSpec: &ppeer.ChaincodeSpec{
	...
	t.Run("BadProposalBytes", func(t *testing.T) {
		...
		require.Error(t, err)
		require.Contains(t, err.Error(), "failed retriving chaincode name from proposal")
		t.Logf("error1 [%s]", err)
	})

	t.Run("BadChaincodeProposalBytes", func(t *testing.T) {
		_, err := protoutil.InvokedChaincodeName(protoutil.MarshalOrPanic(&ppeer.Proposal{
			Payload: []byte("garbage"),
		}))
		require.Error(t, err)
		require.Contains(t, err.Error(), "failed retriving chaincode name from chaincode proposal payload")
		t.Logf("error2 [%s]", err)
	})
	...
	t.Run("BadChaincodeInvocationSpec", func(t *testing.T) {
		...
		require.Error(t, err)
		require.Contains(t, err.Error(), "failed retriving chaincode name from chaincode invocation spec")
		t.Logf("error3 [%s]", err)
	})
	...
	t.Run("NilChaincodeSpec", func(t *testing.T) {
		...
		require.EqualError(t, err, "failed retriving chaincode name, because the chaincode invocation spec is nil")
	})
	...
	t.Run("NilChaincodeID", func(t *testing.T) {
		...
		require.EqualError(t, err, "failed retriving chaincode name, because the chaincode id is nil")
	})
}
